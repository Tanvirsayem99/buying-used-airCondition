...
export const HowItWorks = () => {
  ...
  const renderIcon = (iconName: string) => {
    switch (iconName) {
      case "MessageSquare":
        return <MessageSquare className="w-8 h-8 text-cyan-600" />;
      case "Calculator":
  ...
  return (
    <section id="how-it-works" className="py-20 bg-[#f8fafc] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {t.howItWorks.steps.map((step, idx) => (
            <div
              key={idx}
              className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 border border-slate-200/80 shadow-md shadow-slate-200/50 hover:border-cyan-400 hover:shadow-xl transition-all flex flex-col justify-between relative group"
            >
              {/* Step Big Number Badge */}
              <div className="flex items-center justify-between mb-6">
                <div className="w-16 h-16 rounded-2xl bg-cyan-50 border border-cyan-200 flex items-center justify-center shadow-xs">
